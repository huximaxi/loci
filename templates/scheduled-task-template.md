...
Palace files move between sessions. The folder structure changes. The only stable identifier is the workspace folder (e.g., `my-palace/`).
...
3. Tiered digest: pre-filter → cheap model per-channel → stronger model meta
