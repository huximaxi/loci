        ...
        while let Some(dir) = stack.pop() {
            ...
            for e in rd.filter_map(|e| e.ok()) {
                ...
                for_each_bounded_line(f, |line| {
                    ...
                    let Some(t) = parse_rfc3339(ts) else { return };
                    if t < horizon {
                        return;
                    }
...
mod bounded_line_tests {
    ...
    #[test]
    fn oversized_line_between_normal_lines() {
        let mut content = b"first\n".to_vec();
        content.extend(vec![b'y'; MAX_LINE_BYTES * 2]);
        content.push(b'\n');
        content.extend_from_slice(b"last\n");
        assert_eq!(scan(&content), vec!["first", "last"]);
    }
}
