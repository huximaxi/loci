...
- `templates/personas/`: a shelf of filled persona examples. Ships with `Cipher.md` (security / adversarial) + `Praxis.md` (sysadmin / reversibility), a paired engine-room dyad showing the theoria/praxis split. Generic, no organisation-specific lock-in; fork into your palace and adapt.
- `templates/skills/`: a shelf of portable disciplines with stated triggers, procedures, and kill conditions. Ships with `quarantine.md`, the long-form procedure behind the v1.4 "Foreign-process quarantine, numbered" doctrine.
